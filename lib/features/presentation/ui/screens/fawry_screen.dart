...
import 'package:cowpay/features/presentation/ui/generic_views/error_alert_view.dart';
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: BlocListener<CowpayBloc, CowpayState>(
          listenWhen: (prev, state) {
            return prev.failure != state.failure;
          },
          listener: (context, state) {
            Failure? stateFailure = state.failure;
            if (stateFailure != null) {
              ErrorAlertView alertView = ErrorAlertView(
                  context: context, content: stateFailure.message ?? "");
              alertView.ackAlert();
            }
          },
          child: Column(
